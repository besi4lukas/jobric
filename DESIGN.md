...
Last updated: 2026-09-14 · reflects `main` @ `9a5e965`, plus this uncommitted
change (Applications PR A of three: `GET`/`PATCH /api/applications`, the
manual status pin, migration `0005_applications.sql`)
...
| `OrchestratorAgent`       | The 9-step pipeline; **all** D1 writes                    | D1                       |
...
```
 ...
 SELECT * FROM email_accounts WHERE last_history_id IS NOT NULL
    ...
    │  │                                              │
    │  │  3b. status_source = 'user' (pinned)         │  ← no tracker call,
    │  │       → touch last_activity_at, COALESCE     │    no events row —
    │  │         interview_at, record message (6),    │    see "Manual
    │  │         return 200 (status untouched)        │    status pin" below
    │  │                                              │
    ...
    │  │                                              │
    │  │  5. advanceApplication: UPSERT applications  │
    │  │     (status + rank, pin-guarded — see below) │
    │  │                                              │
...
**Manual status pin.** `applications.status_source` (migration
`0005_applications.sql`) records who last set `status`: `'gmail'` for the
pipeline, `'user'` for a `PATCH /api/applications/:id/status` edit from the
dashboard (`db/applications.ts` `setApplicationStatusByUser`). A manual edit
wins until the user edits again — the orchestrator never overwrites a
`'user'` status, and skips the StatusTracker call entirely once it sees the
pin (step 3b) rather than deriving a result it would just discard. The guard
is enforced twice: step 3b is the cost optimization (no LLM spend on a
result that can't land), and `db/applications.ts` `advanceApplication()`
bakes the same check into its `UPDATE`'s SQL (a `CASE WHEN status_source =
'user'`) so a caller that forgets to check can't clobber a pinned row
either. Mail on a pinned application still ingests normally — it's recorded
in the inbox, `last_activity_at` still moves, and `interview_at` can still
pick up a new date via the same `COALESCE` the pipeline always used; only
`status`/`funnel_rank` are frozen. There is currently no "resume automatic
tracking" action (techdebt: cheap follow-up, a `DELETE` on the same route).
...
users (Clerk id)
  ...
  │             │  status_source: 'gmail' | 'user' — who last set status;
  │             │    'user' pins it against the pipeline (§4 "Manual status
  │             │    pin")
  │             │
  │             ├──< events      audit trail: what changed, when, why
  │             │                  message_id → the message that caused it
  │             │                  source: 'gmail' (pipeline) | 'user'
  │             │                  ('user' written by PATCH /api/applications;
...
`GET`/`PATCH /api/applications` now exist (agents-side, "Applications PR A of
three" below) but nothing on the web side calls them yet; that's PR B/C.
...
### The Applications read/write path

Agents-side only ("Applications PR A of three" — the web tab and its fetch
helpers are PR B/C, same split as Inbox). Replaces the orchestrator's old
`/applications` DO endpoint (deleted this PR — it had no caller).

- **`GET /api/applications?limit=&cursor=`** — same shape of decision as
  `/api/inbox`: plain D1 reads registered ahead of `routeAgentRequest`,
  keyset pagination on `(last_activity_at, id)` (migration
  `0005_applications.sql` replaces the 0001 index with one that includes
  `id DESC` so the cursor is served directly), `limit + 1` probe for
  `nextCursor` instead of a COUNT. The cursor codec moved out of
  `routes/inbox.ts` into `lib/keyset-cursor.ts` so both routes share it.
  `emailCount` is `SUM(threads.message_count)` across every thread bound to
  the application, not a second join over `messages` — `message_count` is
  already a derived, replay-safe column (§4), so summing it stays correct
  for free.
- **`PATCH /api/applications/:id/status`** — the manual status pin (§4
  "Manual status pin"). Body `{ status }`; 404 (never a 403 or a
  distinguishable error) when the id doesn't belong to the caller. Sets
  `status_source='user'`, bumps `last_activity_at`, and — only when the
  status actually changed — writes one `events` row with `source='user'`,
  the same `event_type` mapping the pipeline uses (`eventTypeForStatus()`,
  now in `db/schema.ts` so both write paths share it), and
  `metadata.previousStatus`. Re-submitting the current status still pins
  (there's now a record of "user confirmed this") but writes no event —
  nothing changed to audit. One `db.batch()`: UPDATE, conditional event
  INSERT, then the same row-shaped SELECT `GET` uses, so PATCH's response
  and a subsequent list page can never disagree about the row's shape.

```
  BUILT & WIRED                          BUILT, NOT WIRED
  ─────────────                          ────────────────
  Gmail OAuth ✓                          Applications UI (mock —
  Cron poll ✓                              _data/companies.ts; web PR B/C
  Agent pipeline ✓                         calls GET/PATCH /api/applications)
